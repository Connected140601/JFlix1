document.addEventListener('DOMContentLoaded', () => {
  ...
  function setupModal() {
    ...
    const playVideoFor = (platform) => {
      ...
      if (platform === 'ios') {
        videoPath = isNoAds ? '/NOADS/images/iOS_Safari_optimized.MP4' : '/images/iOS_Safari_optimized.MP4';
    ...
    androidButton.onclick = () => {
      // When Android button is clicked, open the download link
      window.open('https://median.co/share/noakox#apk', '_blank');
      
      // Visually select the Android button and deselect iOS
      androidButton.classList.add('selected');
      iosButton.classList.remove('selected');
      
      // Hide the video player if it was previously shown for iOS
