const API_KEY = '85d719cee777256ef448976581dbc2cb';
      ...
      switch(currentServer) {
        ...
        case 'vidsrcto':
          if (isAnime || type === MEDIA_TYPES.KOREAN || isCartoon) {
            videoUrl = `${SERVER_URLS[currentServer]}tv/${tmdbId}/1/1`;
          } else {
...
function updateServerNoticesVisibility() {
  ...
  const server4Notice = document.getElementById('server4-notice');

  if (server1Notice) {
    server1Notice.style.display = (currentServer === 'vidsrcxyz' ? 'flex' : 'none');
  }
  if (server2Notice) {
    // Show notice for 'videasy' server instead of 'vidsrcto'
    server2Notice.style.display = (currentServer === 'videasy' ? 'flex' : 'none');
  }
  if (server3Notice) {
    server3Notice.style.display = (currentServer === '2embed' ? 'flex' : 'none');
  }
  if (server4Notice) {
    server4Notice.style.display = (currentServer === 'vidsrcto' ? 'flex' : 'none');
  }
}
...
async function init() {
  ...
  // Setup Server 4 GIF modal listeners
  const server4GifThumb = document.getElementById('server4-instructional-gif-thumb');
  if (server4GifThumb) {
    server4GifThumb.addEventListener('click', openServer4GifModal);
  }

  const server4GifModalElement = document.getElementById('server4-gif-modal');
  if (server4GifModalElement) {
    server4GifModalElement.addEventListener('click', function(event) {
        if (event.target === this) { // Clicked on overlay itself
            closeServer4GifModal();
        }
    });
  }
}
...
// Server 4 GIF Modal Functions
function openServer4GifModal() {
  const modal = document.getElementById('server4-gif-modal');
  if (modal) modal.style.display = 'flex';
}

function closeServer4GifModal() {
  const modal = document.getElementById('server4-gif-modal');
  if (modal) modal.style.display = 'none';
}
