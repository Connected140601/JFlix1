...
async function checkUserSession() {
    ...
    supabase.auth.onAuthStateChange(async (event, currentSession) => {
        ...
        // If handleAccessControl triggers a redirect, stop processing to avoid UI flicker.
        if (await handleAccessControl(authUser)) {
            return;
        }

        // After redirection check, update the UI based on whether a user is present.
        if (authUser) {
            updateUIForLoggedInUser(authUser);
            // Close the login/signup modal only on initial sign-in.
            if (event === 'SIGNED_IN') {
                closeAuthModals();
            }
        } else {
